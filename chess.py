...
import board_game
from board_game import BlackWhitePieceType, Piece, SegType, bw_enemy
...
class Chess:
  ...
  def is_valid_move(self, piece, to):
    ...
    assert not target or target.type != ChessPiece.King

    if target and target.color == color:
      return False

    with board_game.move_virtually(self.board, piece.coords, to):
  ...
  def _is_valid_move_internal(self, piece, to):
    ...
    segment, segtype = board_game.get_segment_cells(self.board, piece.coords, to)
    is_free_segment = board_game.is_free_segment(segment, segtype)
    ...
    if piece.type == ChessPiece.Pawn:
      ...
      if segtype == SegType.LINE and is_free_segment:
        util.list_remove_if_exist(self.unmoved_castlers[color], piece)
        return True
      return False
  ...
  def move(self, piece, to):
    board_game.remove_if_placed(self.board, to)
    board_game.move(self.board, piece.coords, to)
  ...
  def _is_valid_castling(self, piece, to):
    ...
    res = board_game.get_segment_cells(self.board, piece.coords, [rook_x, rook_y])
    if board_game.is_free_segment(*res):
      return False
  ...
  def _castling(self, color):
    if len(self.unmoved_castlers[color]) != 1:
      return
    rook, = self.unmoved_castlers[color]
    x, y = rook.coords
    assert y == self._own_side_y(0, color)
    x2 = 3 if x == 0 else 5
    board_game.move(self.board, rook.coords, [x2, y])
    self.unmoved_castlers[color] = []
